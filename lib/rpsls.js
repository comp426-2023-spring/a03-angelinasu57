...
export function rpslsGame(choice) {
    ...
    if (choice === undefined)
        return {player: rpslsOptions[Math.floor(Math.random()*(choice.length-1))]
    };
  
    choice = choice.toLowerCase();
    if (!rpslsOptions.includes(choice)) {
    ...
    var oppChoice = rpslsOptions[Math.floor(Math.random()*(choice.length-1))];
...
export function rpsGame(choice) {
    ...
    if (choice === undefined)
        return {player: rpsOptions[Math.floor(Math.random()*(choice.length-1))]
    };
    ...
    var oppChoice = rpsOptions[Math.floor(Math.random()*(choice.length-1))];
    return {player: choice, opponent: oppChoice, result: RULES[choice][oppChoice]}
  }
